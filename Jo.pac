...
function isJordan(ip) {
  if (isIPv4(ip)) return isJordanV4(ip);
  if (isIPv6(ip)) return isJordanV6(ip);
  return false;
}
...
function isGulfV4(ip) {
  return (
    ...
    ip.startsWith("213.178.160.")||
    ip.startsWith("213.178.161.")||
    ip.startsWith("213.178.162.")||
    ip.startsWith("213.178.163.")||
    // Kuwait - Zain
    ip.startsWith("212.118.64.") ||
    ip.startsWith("212.118.65.") ||
    ip.startsWith("212.118.66.") ||
    ip.startsWith("212.118.67.") ||
    ip.startsWith("212.118.68.") ||
    ip.startsWith("212.118.69.") ||
    // Kuwait - Ooredoo
    ip.startsWith("78.56.")      ||
    ip.startsWith("78.57.")      ||
    ip.startsWith("78.58.")      ||
...
function isGulfV6(ip) {
  ...
  return (
    ...
    ex.startsWith("2001:df0:448:") ||
    // Saudi Arabia
    ex.startsWith("2a0d:5987:") ||
    ex.startsWith("2a06:8ec1:") ||
    ex.startsWith("2a06:8ec0:") ||
    ex.startsWith("2001:df4:8000:") ||
    // Bahrain
    ex.startsWith("2a01:9880:1:") ||
...
function isGulf(ip) {
  if (isIPv4(ip)) return isGulfV4(ip);
  if (isIPv6(ip)) return isGulfV6(ip);
  return false;
}
...
function FindProxyForURL(url, host) {
  ...
  // ============================================================
  // الأردن أولاً ← أقل بنق + أولوية قصوى
  // الخليج ثانياً ← fallback
  // غيرهم ← BLOCK
  // ============================================================

  if (isJordan(ip)) return PROXY;
  if (isGulf(ip))   return PROXY;

  return BLOCK;
}
// ============================================================
// END - PUBG JORDAN + GULF LOCK v9.0
// ============================================================
